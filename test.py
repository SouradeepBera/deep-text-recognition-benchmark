...
def benchmark_all_eval(model, criterion, converter, opt, calculate_infer_time=False):
    ...
    for eval_data in eval_data_list:
        ...
        _, accuracy_by_best_model, norm_ED_by_best_model, _, _, _, infer_time, length_of_data, _ = validation(
...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    with profile(
        ...
        profile_memory=True,
        with_flops=True,
        schedule=torch.profiler.schedule(wait=PROFILER_WAIT_STEPS, warmup=PROFILER_WARMUP_STEPS, active=PROFILER_ACTIVE_STEPS, repeat=PROFILER_REPEAT_CYCLES),
        acc_events=True
    ) as prof:
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            evaluation_loader = torch.utils.data.DataLoader(
                ...
                num_workers=int(opt.workers),
                collate_fn=AlignCollate_evaluation, pin_memory=True)
            _, accuracy_by_best_model, _, _, _, _, _, _, prof = validation(
                model, criterion, evaluation_loader, converter, opt)
            log.write(eval_data_log)
